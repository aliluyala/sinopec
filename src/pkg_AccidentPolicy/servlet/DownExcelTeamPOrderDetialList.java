...
/**
 * Servlet implementation class DownExcelTeamPOrderDetialList
 */
@WebServlet("/DownExcelTeamPOrderDetialList.do")
public class DownExcelTeamPOrderDetialList extends HttpServlet {
	private static final long serialVersionUID = 1L;
	private static Logger log = Logger
	.getLogger(DownExcelTeamPOrderDetialList.class);   
    /**
     * @see HttpServlet#HttpServlet()
     */
    public DownExcelTeamPOrderDetialList() {
        super();
	...
	private void export_POrderList(HttpServletRequest request,
		...
		String team_code = (String) request.getParameter("teamcode");
		int teamcode = Integer.parseInt(team_code);
		...
		try {
			...
			ws.addCell(new Label(0, 0,  team.getName()+"简易险出单明细表", wcf_title));
			ws.mergeCells(0, 0, 8, 0);
			ws.addCell(new Label(0, 1,  "日期："+qrq+"至"+zrq,wcf_head));
			ws.mergeCells(0, 1, 8, 1);
			ws.addCell(new Label(0, 2, "序号", wcf_head));
			ws.addCell(new Label(1, 2, "简易险产品", wcf_head));
			ws.addCell(new Label(2, 2, "保险公司", wcf_head));
			ws.addCell(new Label(3, 2, "份数", wcf_head));
			ws.addCell(new Label(4, 2, "金额(元)", wcf_head));
			ws.addCell(new Label(5, 2, "出单机构", wcf_head));
			ws.addCell(new Label(6, 2, "机构名", wcf_head));
			ws.addCell(new Label(7, 2, "出单时间", wcf_head));
			ws.addCell(new Label(8, 2, "发展人", wcf_head));
			List<PolicyOrder> plist = pL.getTeamSuccessList(sql,teamcode);
			...
			for(int r=0;r<size;r++) {
				...
				ws.addCell(new Label(1, s_line + r,plist.get(r).getPolicy_name(), wcf_head));
				ws.addCell(new Label(2, s_line + r,zd.GetBxgs(bxgs), wcf_head));
				ws.addCell(new jxl.write.Number(3, s_line + r,plist.get(r).getPolicy_num(), wcf_table_numz));
				ws.addCell(new jxl.write.Number(4, s_line + r,plist.get(r).getSum_price(), wcf_table_num));
				ws.addCell(new Label(5, s_line + r,plist.get(r).getOrg_code(), wcf_head));
				ws.addCell(new Label(6, s_line + r,plist.get(r).getOrg_code_name(), wcf_head));
				ws.addCell(new Label(7, s_line + r,plist.get(r).getSproce_time(), wcf_head));
				ws.addCell(new Label(8, s_line + r,zd.GetCzyname(plist.get(r).getOpercode(),
						plist.get(r).getOrg_code()), wcf_head));
			}
			 map = pL.getTeamSumPrice(sql, teamcode);
				...
				ws.addCell(new Label(3, s_line + size,map.get("num"), wcf_head));
				ws.addCell(new Label(4, s_line + size,map.get("price"), wcf_head));
	...
	private String getFileName() {
		this.setFileName("团队简易险出单明细表" + ".xls");
		return fileName;
	}
