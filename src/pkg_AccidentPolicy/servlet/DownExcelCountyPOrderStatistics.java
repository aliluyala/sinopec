...
public class DownExcelCountyPOrderStatistics extends HttpServlet {
	...
	private void export_POrderList(HttpServletRequest request,
		...
		try {
			...
			WritableSheet ws = wwb.createSheet("简易险出单通报(" + qrq+ "至"+zrq+")", 0);
			// 定义单元格样式
			...
			ws.addCell(new Label(0, 0,  qxname+"简易险出单通报表", wcf_title));
			...
			ws.addCell(new Label(2, 2, "保费计划(元)", wcf_head));
			ws.addCell(new Label(3, 2, "份数", wcf_head));
			ws.addCell(new Label(4, 2, "金额(元)", wcf_head));
			ws.addCell(new Label(5, 2, "保费完成进度", wcf_head));
	...
	private String getFileName() {
		this.setFileName("区县团队简易险出单通报表" + ".xls");
		return fileName;
	}
