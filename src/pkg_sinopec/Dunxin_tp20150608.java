 ...
 * 
 */
public class Dunxin_tp20150608 {
	private static Logger log = Logger.getLogger(Dunxin_tp20150608.class);
	//private static String wj = "C:/jboss6/server/default/deploy/sinopec.war/sendmsg_tp.ini";
	 private static String wj="/opt/weblogic/user_projects/domains/base_domain/servers/bmzww/sinopec/sendmsg_tp.ini";
	public  String SendXx(String hm, String nr, String citycode)
		...
		if (!st.equals("")) {
			...
			nvps.add(new BasicNameValuePair("userCode", rd.getHashValue(
					citycode, "PAccount", wj)));
			nvps.add(new BasicNameValuePair("userPass", rd.getHashValue(
			...
			rets = "-23";
		if (rets.startsWith("-") || "0".equals(rets))
			rets = dxfhdm(rets);
		else
			rets = "短信发送成功。[" + rets + "]";
		return rets;
	...
	private static String dxfhdm(String aa) {
		String s = "";
		if (aa.startsWith("-6:")) {
