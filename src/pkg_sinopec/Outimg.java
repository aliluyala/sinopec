...
public class Outimg {
  ...
  public void Creat_img(int xh,String courseFile) throws Cls_exception{
	   ...
	   try {
    	...
    	stmt = con.prepareStatement("select PHOTO from kcs_specphoto where serial_no=?");
		stmt.setInt(1, xh);
  ...
  public String getstring(int xh) throws Cls_exception {
		...
		try{	
			...
			conn = cn.connect().getConnection();
 		    pstmt=conn.prepareStatement("select dd_id,mark,zjh from kcs_specphoto where serial_no="+xh);
       ...
       str=xh+"_"+mark+".jpg";
	   log.error("序列号："+xh+" 文件名："+str);
		return str;
	} 
  /*
   * 订单号不为0 没有有证件号的文件,保存为: 订单号_mark
   */
  public String getstring0(int xh) throws Cls_exception {
		String a="",b="",str="";
		int mark=0;
		java.sql.Connection conn=null;
		java.sql.PreparedStatement pstmt=null;
